<template>
    <main class="app-content">
         <div class="app-title row">
             ...
             <div class="col-md-4">
                 <select class="form-select" @change="onChangeUnidad($event)" v-model="selectUnidad">
...
export default {
  ...
  methods: {
    ...
    listarActivos(cod_cont, cod_aux, criterio, buscar){
        let me=this;
        if(this.idunidad==2){
             var url= '/actuales/auxiliar?codcont='+ cod_cont +'&codaux=' + cod_aux +'&buscar='+ buscar + '&criterio='+ criterio;
        axios.get(url).then( (response) =>{
            var respuesta= response.data;
        ...
        .catch( (error)=> {
            ...
            if(this.idunidad==3){
                var url= '/bajas/auxiliar?codcont='+ cod_cont +'&codaux=' + cod_aux +'&buscar='+ buscar + '&criterio='+ criterio;
        axios.get(url).then( (response) =>{
            var respuesta= response.data;
            me.arrayActivos = respuesta.bajas;
            me.totalActivos = respuesta.totalbajas;
        })
        .catch( (error)=> {
            console.log(error);
        });
